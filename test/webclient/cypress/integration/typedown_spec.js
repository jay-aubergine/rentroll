...
// --- Setup --
const section = require('../support/components/typedown_test'); // Chart of accounts
...
describe('AIR Roller UI Tests - Chart of accounts', function () {
    ...
    ************************************************************/
    it('Check default value of fields for new record form', function () {
        ...
        cy.get('li[data-target="#people"] .step').click().wait(constants.WAIT_TIME);
        cy.get('#Transactant').siblings('.w2ui-field-helper').click().focused().type('J').wait(constants.WAIT_TIME);
        cy.screenshot('after typing');
        cy.get('#w2ui-overlay tr[index=0]').click();
        cy.screenshot('after overlay');
        cy.get('#Transactant').siblings('button').click();
    });
